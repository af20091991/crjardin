-- Helper: get or create a stable unsubscribe token for an email
...
-- Rewrite add_client_message to include an unsubscribe_token in the enqueued email
CREATE OR REPLACE FUNCTION public.add_client_message(p_token text, p_intervention_id uuid, p_kind text, p_content text, p_author_name text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  BEGIN
    ...
    IF v_owner_email IS NOT NULL THEN
      v_unsub := public.get_or_create_unsubscribe_token(v_owner_email);
      v_html :=
...
-- Rewrite handle_new_user to include an unsubscribe_token in the enqueued email
...
BEGIN
  ...
  IF NOT v_is_admin THEN
    ...
    BEGIN
      v_unsub := public.get_or_create_unsubscribe_token('fournier.anthony2009@gmail.com');
      v_html :=
        '<div style="font-family:Garamond,Georgia,serif;background:#ffffff;padding:24px;max-width:600px;margin:0 auto;color:#2f3a26">'
        || '<p style="font-size:22px;font-weight:700;color:#4F8E33;margin:0;text-align:center">De la graine au jardin</p>'
        || '<p style="font-size:14px;color:#EE8627;margin:2px 0 16px;font-style:italic;text-align:center">au rythme de la nature</p>'
