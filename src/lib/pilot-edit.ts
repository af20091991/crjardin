...
export const DATASETS: DatasetDef[] = [
  ...
  },
  {
    ...
    question: "Les heures réellement passées sur mes chantiers sont-elles saisies ?",
    table: "interventions",
    columns: [
      { key: "intervention_date", label: "Date", type: "date", editable: true },
      { key: "title", label: "Intitulé", type: "text", editable: true, width: "min-w-[16rem]" },
      { key: "intervention_type", label: "Type", type: "text", editable: true },
      { key: "hours_spent", label: "Heures réalisées", type: "number", editable: true },
      { key: "internal_hourly_rate", label: "Coût horaire interne", type: "number", editable: true },
      { key: "status", label: "Statut", type: "text" },
      { key: "reference", label: "Référence", type: "text" },
    ],
    searchFields: ["title", "reference", "intervention_type"],
    orderBy: [{ column: "intervention_date", ascending: false }],
    impacts: ["Taux horaire", "Rentabilité clients", "Analyse Temps & Rentabilité", "Qualité des données"],
    labelOf: (r) => String(r.title ?? r.reference ?? "Intervention"),
  },
  {
    id: "heures_historiques",
    label: "Heures historiques",
    question: "Mon historique d'heures importé est-il correctement attribué ?",
    table: "pilot_historic_hours",
    columns: [
      { key: "year", label: "Année", type: "number" },
      { key: "raw_client_text", label: "Libellé d'origine", type: "text", width: "min-w-[16rem]" },
      { key: "hours", label: "Heures", type: "number", editable: true },
      { key: "amount_ht", label: "Montant HT", type: "number", editable: true },
      { key: "margin_net", label: "Marge nette", type: "number", editable: true },
      { key: "confidence", label: "Confiance", type: "select", editable: true, options: ["haute", "moyenne", "faible"] },
      { key: "status", label: "Statut", type: "select", editable: true, options: ["valide", "a_valider", "non_attribue"] },
      { key: "note", label: "Note", type: "text", editable: true, width: "min-w-[14rem]" },
    ],
    searchFields: ["raw_client_text", "note"],
    orderBy: [
      { column: "year", ascending: false },
      { column: "raw_client_text", ascending: true },
    ],
    impacts: ["Historique heures", "Qualité des données"],
    labelOf: (r) => `${r.raw_client_text ?? "Historique"} (${r.year ?? "?"})`,
  },
  {
    id: "clients_legacy_placeholder",
    label: "Clients (doublon)",
    question: "",
    table: "clients",
    columns: [
      { key: "name", label: "Nom", type: "text", editable: true, width: "min-w-[14rem]" },
