...
export function FichesIndexRoute() {
  return (
...
export function FichesIndex() {
  ...
  const filtered = useMemo(() => {
    const now = new Date();
    let list = [...(sheets ?? [])];
    if (intervenant !== "all") list = list.filter((s) => parseWorksiteIntervenants(s.intervenant).includes(intervenant));
    if (period !== "all") {
      list = list.filter((s) => {
    ...
    if (q) {
      list = list.filter((s) =>
        [name(s), s.address, ...parseWorksiteIntervenants(s.intervenant), s.intervenant, s.contact_person, ...s.tasks].some((f) =>
    ...
    <div className="mx-auto max-w-5xl space-y-4">
      <div className="space-y-3 rounded-xl border border-border bg-card p-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Select value={intervenant} onValueChange={(v) => { setIntervenant(v); setLimit(PAGE); }}>
            <SelectTrigger className="h-8 w-44"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">Tous les intervenants</SelectItem>
              {INTERVENANTS.map((i) => <SelectItem key={i} value={i}>{i}</SelectItem>)}
            </SelectContent>
          </Select>
          <Select value={period} onValueChange={(v) => { setPeriod(v as Period); setLimit(PAGE); }}>
            <SelectTrigger className="h-8 w-44"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          {hasFilters && (
            <Button size="sm" variant="ghost" className="h-8" onClick={() => { setSearch(""); setIntervenant("all"); setPeriod("all"); }}>
      ...
      {isLoading ? (
        ...
        <div className="space-y-5">
          {groups.map(([key, rows]) => (
            <section key={key} className="space-y-1.5">
              <div className="flex items-baseline justify-between px-1">
                <h2 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{sort === "client" ? key : monthLabel(key)}</h2>
                <span className="text-[11px] text-muted-foreground">{rows.length}</span>
              </div>
              <div className="divide-y divide-border overflow-hidden rounded-xl border border-border bg-card">
                {rows.map((s) => (
                  <Link key={s.id} to="/fiches/$ficheId" params={{ ficheId: s.id }} className="flex items-center gap-3 px-3 py-2.5 transition-colors hover:bg-accent/40">
                    <div className="w-24 shrink-0 text-xs tabular-nums text-muted-foreground">{s.intervention_date ? new Date(s.intervention_date).toLocaleDateString("fr-FR") : "—"}</div>
                    ...
                    {parseWorksiteIntervenants(s.intervenant).map((name) => <Badge key={name} variant="secondary" className="shrink-0">{name}</Badge>)}
          ...
          {visible.length < filtered.length && (
            <div className="flex justify-center pt-1"><Button variant="outline" onClick={() => setLimit((l) => l + PAGE)}>Afficher {Math.min(PAGE, filtered.length - visible.length)} de plus</Button></div>
          )}
        </div>
      )}
    </div>
