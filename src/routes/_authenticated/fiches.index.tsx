...
export function FichesIndex() {
  ...
  const filtered = useMemo(() => {
    ...
    let list = [...(sheets ?? [])];
    if (intervenant !== "all")
      list = list.filter((s) => parseWorksiteIntervenants(s.intervenant).includes(intervenant));
    if (period !== "all") {
      list = list.filter((s) => {
    ...
    if (q) {
      list = list.filter((s) =>
        [
          name(s),
          s.address,
          ...parseWorksiteIntervenants(s.intervenant),
          s.intervenant,
          s.contact_person,
          ...s.tasks,
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center gap-3 py-10 text-center">
            ...
            <Link to="/fiches/new">
              <Button>
                <Plus className="mr-1.5 h-4 w-4" /> Créer une fiche
              </Button>
            </Link>
          </CardContent>
        </Card>
              ...
              <div className="divide-y divide-border overflow-hidden rounded-xl border border-border bg-card">
                {rows.map((s) => (
                  ...
                  >
                    ...
                    {parseWorksiteIntervenants(s.intervenant).map((name) => (
