...
function ClientsPage() {
  ...
  const favoriteMutation = useMutation({
    ...
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["favorite-clients"] }),
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    for (const entry of entriesQ.data ?? []) {
      if (
        !entry.client_id ||
        new Date(entry.entry_date).getFullYear() !== year
      ) {
        continue;
      }
      ...
      ca.set(
        entry.client_id,
        (ca.get(entry.client_id) ?? 0) + (Number(entry.amount_ht) || 0),
      );
      const previous = last.get(entry.client_id);
      if (!previous || entry.entry_date > previous) {
        last.set(entry.client_id, entry.entry_date);
      }

      if (saleRateEligible(entry)) {
        hours.set(
          entry.client_id,
          (hours.get(entry.client_id) ?? 0) + (Number(entry.hours) || 0),
        );
        rated.set(
          entry.client_id,
          (rated.get(entry.client_id) ?? 0) +
            (Number(entry.amount_ht) || 0),
        );
      }
    }

    return (clientsQ.data ?? []).map((client) => {
      const h = hours.get(client.id) ?? 0;
      const date = last.get(client.id) ?? null;

      return {
        client,
        ca: ca.get(client.id) ?? 0,
        hours: h,
        lastDate: date,
        hourlyRate: hourlyRate(rated.get(client.id) ?? 0, h),
        activity:
          client.lifecycle_status === "perdu"
            ? "perdu"
            : getClientActivityStatus(date),
      };
    });
  ...
  const suspects = useMemo(
    () => findSuspectClients(clientsQ.data ?? []),
    [clientsQ.data],
  );
          ...
          <Tabs defaultValue="cleaning" className="pt-1">
            <TabsList className="h-9">
              <TabsTrigger value="cleaning">
                À vérifier
                <span className="ml-1 text-xs">{suspects.length}</span>
              </TabsTrigger>
            </TabsList>
            <TabsContent value="cleaning">
              <div className="rounded-xl border border-dashed bg-muted/10 p-4">
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  {suspects.slice(0, 8).map(
                    ({ client, reason, suggestion }) => (
                      <div
                        key={client.id}
