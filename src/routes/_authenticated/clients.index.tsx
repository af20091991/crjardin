...
import { AlertTriangle, ChevronDown, Filter, Merge, Plus, Star, Upload } from "lucide-react";
...
import { listClients, updateClient, type Client, type ClientLifecycle, type ReportPolicy } from "@/lib/clients";
...
function ClientsPage() {
  ...
  const recosQ = useQuery({ queryKey: ["recommendations-all"], queryFn: listAllRecommendations });
  const favorites = useMemo(() => new Set(favoritesQ.data ?? []), [favoritesQ.data]);
  const stale = useMemo(() => staleClientIds(recosQ.data ?? []), [recosQ.data]);
  ...
  const statusMutation = useMutation({ mutationFn: ({ client, patch }: { client: Client; patch: { lifecycle_status?: ClientLifecycle; report_policy?: ReportPolicy } }) => updateClient(client.id, { name: client.name, ...patch }), onSuccess: () => { toast.success("Fiche client mise à jour"); void qc.invalidateQueries({ queryKey: ["clients"]]); }, onError: (e: Error) => toast.error(e.message) });
  ...
  const filtered = useMemo(() => {
    ...
    const result = rows.filter((row) => {
      const c = row.client;
      if (status === "cr_a_qualifier") { if ((c.report_policy ?? "a_confirmer") !== "a_confirmer") return false; } else if (status !== "all" && row.activity !== status) return false;
      if (!showLost && status !== "perdu" && c.lifecycle_status === "perdu") return false;
      return !q || [c.name, c.address, c.email, c.phone, c.contract_type].filter(Boolean).some((v) => v!.toLowerCase().includes(q));
    });
  ...
  return <AppShell title="Clients"><div className="mx-auto w-full max-w-[1400px] space-y-5">
    ...
    <div className="flex flex-wrap items-center gap-2"><div className="flex rounded-lg border bg-background p-1 text-sm"><button onClick={() => setStatus("all")} className={`rounded-md px-3 py-1.5 ${status === "all" ? "bg-muted font-medium" : "text-muted-foreground"}`}>Tous <span className="ml-1 text-xs text-muted-foreground">{rows.length}</span></button><button onClick={() => setStatus("actif")} className={`rounded-md px-3 py-1.5 ${status === "actif" ? "bg-muted font-medium" : "text-muted-foreground"}`}>Actifs</button><button onClick={() => setStatus("a_relancer")} className={`rounded-md px-3 py-1.5 ${status === "a_relancer" ? "bg-muted font-medium" : "text-muted-foreground"}`}>À relancer</button><button onClick={() => setStatus("dormant")} className={`rounded-md px-3 py-1.5 ${status === "dormant" ? "bg-muted font-medium" : "text-muted-foreground"}`}>Dormants</button></div><Select value={sort} onValueChange={(v) => setSort(v as SortKey)}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="name">Trier : nom</SelectItem><SelectItem value="ca">Trier : CA</SelectItem><SelectItem value="recent">Trier : activité</SelectItem></SelectContent></Select><label className="ml-auto flex items-center gap-2 rounded-lg border px-3 py-2 text-xs text-muted-foreground"><Switch checked={showLost || status === "perdu"} disabled={status === "perdu"} onCheckedChange={setShowLost} />Perdus ({lostCount})</label></div>
