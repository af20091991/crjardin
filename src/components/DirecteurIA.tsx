...
const modes: { id: AdppMode; label: string; icon: typeof Brain; hint: string }[] = [
  { id: "direction", label: "Direction", icon: Lightbulb, hint: "Analyser, décider, conseiller" },
  {
    ...
    label: "Améliorer PP",
    icon: Wrench,
    hint: "Proposer une évolution de Pilot Pro",
  },
];
...
export function DirecteurIA() {
  ...
  return (
    <section
      ...
      className={`fixed z-[100] flex flex-col overflow-hidden border border-border/80 bg-background/98 shadow-2xl backdrop-blur-xl transition-all duration-200 ${maximized ? "inset-4 rounded-2xl" : "bottom-5 right-5 h-[min(760px,calc(100vh-40px))] w-[min(760px,calc(100vw-40px))] rounded-2xl"}`}
    >
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {currentMode.label} · {context.pageTitle}
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-1 overflow-x-auto">
          {modes.map(({ id, label, icon: Icon }) => (
            <button
              ...
              type="button"
              onClick={() => setMode(id)}
              className={`inline-flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition ${mode === id ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
            >
          ...
          <div className="mx-auto flex h-full max-w-2xl flex-col justify-center">
            <div className="mb-6">
              ...
              <h2 className="mt-2 text-2xl font-semibold tracking-tight">
                Que voulez-vous savoir ou améliorer ?
              </h2>
              <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
                Je peux raisonner, analyser, calculer, rechercher et préparer des évolutions de
                Pilot Pro. Les détails techniques restent masqués.
              </p>
            </div>
            <div className="grid gap-2 sm:grid-cols-3">
              {suggestions[mode].map(({ label, icon: Icon }) => (
                <button
                  key={label}
                  type="button"
                ...
                <div
                  className={
                    message.role === "user"
                      ...
                      : "max-w-[92%] rounded-2xl rounded-bl-md border border-border/70 bg-muted/30 px-4 py-3 text-sm leading-6 text-foreground"
                  }
                >
                  ...
                  {message.actions && message.actions.length > 0 ? (
                    <div className="mt-3 space-y-2">
                      {message.actions.map((action) => (
                        <div
                          key={action.id}
                          className="flex items-center justify-between gap-3 rounded-lg border border-border/70 bg-background/70 p-2.5"
                        >
                          <div>
                          ...
                          <button
                            type="button"
                            disabled
                            className="inline-flex shrink-0 items-center gap-1 rounded-md bg-muted px-2.5 py-1.5 text-[11px] text-muted-foreground"
                          >
      ...
      <footer className="shrink-0 border-t border-border/70 p-3">
        <div className="rounded-xl border border-border bg-background focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10">
          <textarea
            ...
            placeholder={`Demandez quelque chose · ${currentMode.hint}…`}
            rows={2}
          ...
          <div className="flex items-center justify-between px-2 pb-2">
            ...
            <button
              ...
              aria-label="Envoyer"
              className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-primary-foreground transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-40"
            >
