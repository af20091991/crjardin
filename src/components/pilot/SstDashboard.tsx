...
export function SstDashboard() {
  ...
  }), [missions, ssts, clients, mode, year]);
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary">{period === "exercice_complet" ? "Exercice complet" : "À date"}</Badge>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">Pilotage économique de la sous-traitance · {year}</p>
        </div>
      ...
      {period === "exercice_complet" && isCurrentYear && (
        <div className="rounded-lg border border-dashed bg-muted/30 px-4 py-2 text-xs text-muted-foreground">
          Les missions datées après aujourd’hui sont incluses dans l’exercice complet et restent des données prévisionnelles.
        </div>
      )}
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.65fr)_minmax(320px,1fr)]">
        <Card>
          <CardHeader><CardTitle>CA / Charge / Marge</CardTitle></CardHeader>
          <CardContent>
            {months.length === 0 ? <p className="py-10 text-center text-sm text-muted-foreground">Aucune donnée sur la période.</p> : <div className="space-y-4">
              {months.map((m) => {
                const max = Math.max(1, m.revenue, m.cost, Math.abs(m.margin));
                return <div key={m.key} className="grid grid-cols-[70px_1fr_90px] items-center gap-3 text-sm">
                  <span className="font-medium">{m.key.slice(5)}/{m.key.slice(0,4)}</span>
                  <div className="space-y-1">
                    <div className="h-2 rounded bg-muted"><div className="h-2 rounded bg-primary" style={{ width: `${Math.min(100, (m.revenue / max) * 100)}%` }} /></div>
                    <div className="h-2 rounded bg-muted"><div className="h-2 rounded bg-foreground/40" style={{ width: `${Math.min(100, (m.cost / max) * 100)}%` }} /></div>
                    <div className="h-2 rounded bg-muted"><div className="h-2 rounded bg-foreground/70" style={{ width: `${Math.min(100, (Math.max(0, m.margin) / max) * 100)}%` }} /></div>
                  </div>
                  <div className="text-right text-xs"><div>{eur(m.revenue)}</div><div className="text-muted-foreground">{eur(m.cost)}</div><div className="font-medium">{eur(m.margin)}</div></div>
                </div>;
              })}
              <div className="flex gap-4 border-t pt-3 text-xs text-muted-foreground"><span>CA</span><span>Charge</span><span>Marge</span></div>
            </div>}
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle>Répartition des charges</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {providers.length === 0 ? <p className="py-10 text-center text-sm text-muted-foreground">Aucune charge SST.</p> : providers.map((p) => <div key={p.key} className="space-y-1"><div className="flex justify-between text-sm"><span>{p.key}</span><span>{eur(p.cost)}</span></div><div className="h-2 rounded bg-muted"><div className="h-2 rounded bg-primary" style={{ width: `${(p.cost / maxCharge) * 100}%` }} /></div></div>)}
          </CardContent>
        </Card>
      </div>

      <Card>
        <CardHeader><CardTitle>Performance des sous-traitants</CardTitle></CardHeader>
        <CardContent className="p-0">
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="border-b bg-muted/30"><tr><th className="px-5 py-3 text-left">Sous-traitant</th><th className="px-3 py-3 text-right">Missions</th><th className="px-3 py-3 text-right">Charge SST</th><th className="px-3 py-3 text-right">CA client</th><th className="px-3 py-3 text-right">Marge</th><th className="px-5 py-3 text-right">Marge %</th></tr></thead><tbody>{providers.map((p) => <tr key={p.key} className="border-b last:border-0"><td className="px-5 py-3 font-medium">{p.key}</td><td className="px-3 py-3 text-right">{p.missions}</td><td className="px-3 py-3 text-right">{eur(p.cost)}</td><td className="px-3 py-3 text-right">{eur(p.revenue)}</td><td className="px-3 py-3 text-right font-medium">{eur(p.margin)}</td><td className="px-5 py-3 text-right">{p.marginPct == null ? "—" : `${p.marginPct.toFixed(1)} %`}</td></tr>)}</tbody></table></div>
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle>Top 5 clients les plus sous-traités</CardTitle></CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">{clientsTop.map((c, i) => <div key={c.key} className="rounded-lg border p-4"><div className="flex items-center justify-between"><span className="text-lg font-semibold">#{i + 1}</span><ArrowUpRight className="h-4 w-4 text-muted-foreground" /></div><div className="mt-2 truncate font-medium">{c.key}</div><div className="mt-1 text-sm text-muted-foreground">{c.missions} mission(s) · {eur(c.cost)} de charge SST</div></div>)}</CardContent>
      </Card>
