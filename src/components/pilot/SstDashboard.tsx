...
export function SstDashboard() {
  ...
  const rows = useMemo(
    () => sstRows({ missions, ssts, clients, mode, year, includeArchived: false }),
    ...
    <div className="space-y-5">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary">
              {period === "exercice_complet" ? "Exercice complet" : "À date"}
            </Badge>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            Pilotage économique de la sous-traitance · {year}
          </p>
        </div>
      ...
      {period === "exercice_complet" && isCurrentYear && (
        <div className="rounded-lg border border-dashed bg-muted/30 px-4 py-2 text-xs text-muted-foreground">
          Les missions datées après aujourd’hui sont incluses dans l’exercice complet et restent des
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">CA SST</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-semibold">{eur(totals.revenue)}</div>
            <p className="text-xs text-muted-foreground">
              CA facturé client associé aux missions SST
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Charge SST</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-semibold">{eur(totals.cost)}</div>
            <p className="text-xs text-muted-foreground">Charge SST enregistrée sur les missions</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Marge SST</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-semibold">{eur(totals.margin)}</div>
            <p className="text-xs text-muted-foreground">
              {totals.marginPct == null ? "—" : `${totals.marginPct.toFixed(1)} % de marge`}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              Client le plus sous-traité
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="truncate text-xl font-semibold">{topClient?.key ?? "—"}</div>
            <p className="text-xs text-muted-foreground">
              {topClient ? `${topClient.missions} mission(s) SST` : "Aucune mission"}
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b bg-muted/30">
                <tr>
                  <th className="px-5 py-3 text-left">Sous-traitant</th>
                  <th className="px-3 py-3 text-right">Missions</th>
                  <th className="px-3 py-3 text-right">Charge SST</th>
                  <th className="px-3 py-3 text-right">CA client</th>
              ...
              <tbody>
                {providers.map((p) => (
                  <tr key={p.key} className="border-b last:border-0">
                    <td className="px-5 py-3 font-medium">{p.key}</td>
                    <td className="px-3 py-3 text-right">{p.missions}</td>
                    <td className="px-3 py-3 text-right">{eur(p.cost)}</td>
                    <td className="px-3 py-3 text-right">{eur(p.revenue)}</td>
                    <td className="px-3 py-3 text-right font-medium">{eur(p.margin)}</td>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          {clientsTop.map((c, i) => (
            <div key={c.key} className="rounded-lg border p-4">
              ...
              <div className="mt-2 truncate font-medium">{c.key}</div>
              <div className="mt-1 text-sm text-muted-foreground">
                {c.missions} mission(s) · {eur(c.cost)} de charge SST
              </div>
            </div>
          ))}
        </CardContent>
      ...
      />
      <p className="text-xs text-muted-foreground">
        Règle économique : CA client − Charge SST = marge SST. Les données futures du mode «
        Exercice complet » sont incluses dans les agrégats mais signalées comme prévisionnelles.
      </p>
    </div>
  );
}
