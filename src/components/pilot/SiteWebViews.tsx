...
const METRIC_META: Record<MetricKey, { label: string; color: string; axis: "left" | "right"; format: (v: number) => string }> = {
  clicks: { label: "Clics", color: "var(--pp-sales)", axis: "left", format: (v) => formatNumber(v) },
  impressions: { label: "Impressions", color: "var(--primary)", axis: "left", format: (v) => formatNumber(v) },
  position: { label: "Position moyenne", color: "var(--pp-warning)", axis: "right", format: (v) => v.toFixed(1).replace(".", ",") },
      ...
      <Card className="p-5">
              ...
              return (
                <button
                  ...
                  className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-colors ${
                    active ? "border-transparent text-primary-foreground" : "border-border text-muted-foreground hover:bg-muted"
                  }`}
        ...
        <div className="mt-4 h-72">
          {loading ? (
            ...
            <EmptyState icon={Search} title="Aucune donnée Search Console disponible sur la période." compact />
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 8, right: 12, left: 0, bottom: 8 }}>
                ...
                {activeMetrics.map((key) => {
                  const meta = METRIC_META[key];
                  return <Line key={key} type="monotone" dataKey={key} name={meta.label} stroke={meta.color} yAxisId={meta.axis} dot={false} />;
                })}
              </LineChart>
            </ResponsiveContainer>
          )}
    ...
    <Card className="border-destructive/30 bg-destructive/5 p-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">{friendlyConnectionError(code) ?? code}</p>
        </div>
      </div>
    </Card>
  );
