...
const actionFor = (line: PendingValidationLine) => {
  if (line.kind === "vente" && line.match_status === "non_identifie") {
    return { accept: "Rattacher ce client", explain: "PP enregistrera le client proposé pour cette vente." };
  }
  if (line.kind === "charge") {
    const reason = line.reasons.find((r) => r === "charge_a_classer" || r === "categorie_incertaine");
    if (reason) {
      const proposed = line.charge_category || "la catégorie proposée";
      return { accept: `Classer comme ${proposed}`, explain: `Cette charge sera enregistrée dans « ${proposed} » et utilisée ainsi dans les calculs de charges.` };
    }
  }
  return { accept: "Confirmer cette donnée", explain: "PP conservera cette donnée comme validée et pourra l'utiliser dans ses calculs." };
};
...
export function ValidationPage() {
  ...
  return <div className="space-y-5">
    <header><h1 className="font-display text-2xl font-semibold">Centre de validation</h1><p className="mt-1 text-sm text-muted-foreground">PP traite les cas certains. Pour les autres, chaque bouton indique exactement ce qu'il va faire.</p></header>
    ...
    <Card><CardContent className="pt-5">
      {isLoading ? <p className="py-10 text-center text-sm text-muted-foreground">Chargement des données…</p> : lines.length === 0 ? <div className="py-10 text-center"><p className="font-medium">Aucune décision nécessaire.</p>{autoSummary && <p className="mt-2 text-sm text-muted-foreground">PP a traité {processed} donnée(s) automatiquement.</p>}</div> : <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Donnée</TableHead><TableHead>Client / classement</TableHead><TableHead className="text-right">Montant</TableHead><TableHead>Ce que PP propose</TableHead><TableHead className="w-[240px] text-right">Décision</TableHead></TableRow></TableHeader><TableBody>{lines.slice(0, 500).map((line) => <ValidationRow key={line.id} line={line} busy={validate.isPending} onValidate={() => validate.mutate({ id: line.id, status: "valide" })} onReview={() => validate.mutate({ id: line.id, status: "a_revoir" })} />)}</TableBody></Table>{lines.length > 500 && <p className="pt-3 text-xs text-muted-foreground">500 premières lignes affichées sur {lines.length}.</p>}</div>}
    </CardContent></Card>
  </div>;
}
...
function ValidationRow({ line, busy, onValidate, onReview }: { line: PendingValidationLine; busy: boolean; onValidate: () => void; onReview: () => void }) {
  ...
  const current = line.kind === "vente" ? (line.match_status === "non_identifie" ? "Client à confirmer" : "Client rattaché") : (line.charge_category || "Classement à confirmer");
  return <TableRow>
    <TableCell className="whitespace-nowrap text-sm">{String(line.month).padStart(2, "0")}/{line.year}</TableCell>
    <TableCell><div className="max-w-[330px]"><div className="truncate font-medium">{line.designation || "Sans libellé"}</div><div className="text-xs text-muted-foreground">{line.kind === "vente" ? "Vente" : "Charge"}</div></div></TableCell>
    <TableCell>{current}</TableCell><TableCell className="text-right tabular-nums">{euro(line.amount_ht)}</TableCell>
    <TableCell><div className="font-medium text-sm">{action.accept}</div><div className="mt-1 text-xs text-muted-foreground">{action.explain}</div></TableCell>
    <TableCell><div className="flex flex-col items-end gap-2"><Button size="sm" onClick={onValidate} disabled={busy} className="w-full gap-1"><Check className="h-4 w-4" /> {action.accept}</Button><Button size="sm" variant="outline" onClick={onReview} disabled={busy} className="w-full gap-1"><X className="h-4 w-4" /> Laisser à revoir</Button></div></TableCell>
  </TableRow>;
}
