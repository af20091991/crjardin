...
import { listNextBestOffers, explainOffer, reasonLabel } from "@/lib/next-best-offers";
...
export function ClientPilotageTabs({ clientId }: { clientId: string }) {
    ...
    <div className="space-y-4">
      <Tabs defaultValue="apercu">
        <TabsList className="w-full">
          ...
          <TabsTrigger value="rentabilite" className="flex-1">
            <Gauge className="mr-1.5 h-4 w-4" />
            Rentabilité
          </TabsTrigger>
          <TabsTrigger value="interventions-facturation" className="flex-1">
            ...
            <Card className="border-amber-200 bg-amber-50/60">
              <CardContent className="flex gap-3 py-4 text-sm">
                ...
                <p className="text-muted-foreground">
                  ...
                  affichés à titre informatif et n'alimentent aucun classement stratégique tant que
                  la fiche n'est pas certifiée dans le Centre de contrôle → Référentiel client.
                </p>
              </CardContent>
            </Card>
          )}
                      ...
                      {(caQ.data ?? []).slice(0, 30).map((r) => (
                        ...
                        >
                          ...
                          <span className="tabular-nums">
                            {formatEuro(Number(r.amount_ht) || 0)}
                          </span>
                        </li>
                      ))}
                    </ul>
        ...
        <TabsContent value="rentabilite" className="space-y-4">
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
                ...
                <MiniStat
                  ...
                  value={
                    score?.realHourlyRate != null ? `${formatEuro(score.realHourlyRate)}/h` : "—"
                  }
                  sub={
          ...
          {score &&
            ...
            (() => {
              ...
              const axes: Array<{
                label: string;
                v: { value: number; max: number; note: string };
              }> = [
        ...
        <TabsContent value="interventions-facturation" className="space-y-4">
          {/* 4 — Historique interventions (facturation) */}
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
                <FileText className="h-4 w-4 text-primary" />
                Historique interventions (facturation)
              </CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  {(interventionsQ.data ?? []).map((iv) => {
                    ...
                    return (
                      <li key={iv.id} className="rounded-lg border border-border p-3">
                        ...
                        {iv.title && (
                          <p className="mt-1 text-sm text-muted-foreground">{iv.title}</p>
                        )}
