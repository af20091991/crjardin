...
function persisted(key: string, fallback: string, allowed: readonly string[]) { const [value,setValue]=useState(fallback); useEffect(()=>{try{const saved=localStorage.getItem(key);if(saved && allowed.includes(saved)) setValue(saved)}catch{}},[key]); const set=(n:string)=>{if(!allowed.includes(n)) return; setValue(n);try{localStorage.setItem(key,n)}catch{}}; return [value,set] as const; }
...
export function CeevFlexibleChart({year,contracts,revenueSeries,breakdown}:Props){
 const[chart,setChart]=persisted("pp:ceev:chart","barres",CHARTS.map(x=>x.value)),[dimension,setDimension]=persisted("pp:ceev:dimension","client",["client","contrat","annee"]),[primary,setPrimary]=persisted("pp:ceev:primary","ca",METRICS.map(x=>x.value)),[secondary,setSecondary]=persisted("pp:ceev:secondary","marge",METRICS.map(x=>x.value));
 const p=primary as Metric,s=secondary as Metric;
 const rows=useMemo(()=>{if(dimension==="client")return breakdown.map(b=>{const cs=contracts.filter(c=>(c.client_name??"Non rattaché")===b.clientName);return{name:b.clientName,primary:cs.reduce((x,c)=>x+value(c,p),0),secondary:cs.reduce((x,c)=>x+value(c,s),0)}});if(dimension==="contrat")return contracts.slice(0,20).map(c=>({name:c.client_name??c.raw_label,primary:value(c,p),secondary:value(c,s)}));return contracts.reduce((acc,c)=>{const key=String(c.year??year);let row=acc.find(x=>x.name===key);if(!row){row={name:key,primary:0,secondary:0};acc.push(row)}row.primary+=value(c,p);row.secondary+=value(c,s);return acc},[] as Array<{name:string;primary:number;secondary:number}>).sort((a,b)=>Number(a.name)-Number(b.name))},[contracts,breakdown,dimension,p,s,revenueSeries]);
 ...
 case"barres":return <BarChart data={rows}>{common}<Bar dataKey="primary" name={`${pn} (${axisUnit(p)})`} fill="var(--pp-sales)" radius={[4,4,0,0]}/></BarChart>;
 case"barres_h":return <BarChart data={rows} layout="vertical"><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number" tick={{fontSize:11}} tickFormatter={v=>tickFormat(p,Number(v))} label={{value:axisUnit(p),position:"insideBottom",offset:-2,fontSize:11}}/><YAxis type="category" dataKey="name" width={120} tick={{fontSize:11}} label={{value:dimension==="client"?"Client":"Contrat",angle:-90,position:"insideLeft",fontSize:11}}/><Tooltip formatter={tooltipFormatter}/><Legend/><Bar dataKey="primary" name={`${pn} (${axisUnit(p)})`} fill="var(--pp-sales)"/></BarChart>;
 case"groupees":return <BarChart data={rows}>{common}<Bar dataKey="primary" name={`${pn} (${axisUnit(p)})`} fill="var(--pp-sales)"/><Bar dataKey="secondary" name={`${sn} (${axisUnit(s)})`} fill="var(--pp-mid)"/></BarChart>;
 case"empilees":return <BarChart data={rows}>{common}<Bar dataKey="primary" name={`${pn} (${axisUnit(p)})`} stackId="a" fill="var(--pp-sales)"/><Bar dataKey="secondary" name={`${sn} (${axisUnit(s)})`} stackId="a" fill="var(--pp-sales)"/></BarChart>;
 case"courbe":return <LineChart data={rows}>{common}<Line type="monotone" dataKey="primary" name={`${pn} (${axisUnit(p)})`} stroke="var(--pp-sales)" strokeWidth={2} dot={{r:3}}/></LineChart>;
 case"aire":return <AreaChart data={rows}>{common}<Area type="monotone" dataKey="primary" name={`${pn} (${axisUnit(p)})`} stroke="var(--pp-sales)" fill="var(--pp-sales)" fillOpacity={.2}/></AreaChart>;
 case"combo":return <ComposedChart data={rows}>{common}<Bar dataKey="primary" name={`${pn} (${axisUnit(p)})`} fill="var(--pp-sales)" radius={[4,4,0,0]}/><Line type="monotone" dataKey="secondary" name={`${sn} (${axisUnit(s)})`} stroke="var(--pp-mid)" strokeWidth={2}/></ComposedChart>;
 case"donut":{const data=rows.map(r=>({name:r.name,value:Math.max(0,r.primary)})).filter(r=>r.value>0);return <PieChart><Pie data={data} dataKey="value" nameKey="name" innerRadius="52%" outerRadius="78%" paddingAngle={2}>{data.map((_,i)=><Cell key={i} fill={i%2?"var(--pp-mid)":"var(--pp-sales)"}/>)}</Pie><Tooltip formatter={(v:number)=>formatValue(p,v)}/><Legend wrapperStyle={{fontSize:11}}/></PieChart>}
 case"radar":return <RadarChart data={rows.slice(0,8)}><PolarGrid/><PolarAngleAxis dataKey="name" tick={{fontSize:10}}/><Radar name={`${pn} (${axisUnit(p)})`} dataKey="primary" stroke="var(--pp-sales)" fill="var(--pp-sales)" fillOpacity={.25}/><Tooltip formatter={(v:number)=>formatValue(p,v)}/></RadarChart>;
 case"nuage":return <ScatterChart><CartesianGrid/><XAxis type="number" dataKey="primary" name={pn} tick={{fontSize:11}} tickFormatter={v=>tickFormat(p,Number(v))} label={{value:`${pn} (${axisUnit(p)})`,position:"insideBottom",offset:-2,fontSize:11}}/><YAxis type="number" dataKey="secondary" name={sn} tick={{fontSize:11}} tickFormatter={v=>tickFormat(s,Number(v))} label={{value:`${sn} (${axisUnit(s)})`,angle:-90,position:"insideLeft",offset:8,fontSize:11}}/><ZAxis range={[40,180]}/><Tooltip cursor={{strokeDasharray:"3 3"}} formatter={(v:number,n:string)=>[formatValue(n===sn?s:p,v),n]}/><Legend/><Scatter name={`${pn} / ${sn}`} data={rows} fill="var(--pp-sales)"/></ScatterChart>;
 }};
