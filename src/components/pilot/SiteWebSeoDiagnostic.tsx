  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="font-normal">Données réelles Google</Badge>
            </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric icon={Crosshair} label="Requêtes visibles" value={loading ? "…" : formatNumber(summary.queryCount)} detail="requêtes ayant généré au moins une ligne Search Console" />
        <Metric icon={ShieldCheck} label="Clics depuis le top 10" value={loading ? "…" : formatPercent(summary.top10ClickShare)} detail="part des clics provenant de positions 1 à 10" />
        <Metric icon={Users} label="Sessions SEO" value={loading ? "…" : formatNumber(organicSessions)} detail="sessions Analytics 4 attribuées à la recherche organique" />
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <SwotCard icon={CheckCircle2} title="Forces" tone="positive">
          {strengths.length ? strengths.map((item) => <QueryLine key={item.query} item={item} suffix="bien positionnée" />) : <EmptyState icon={CheckCircle2} title="Pas encore assez de données" compact />}
        </SwotCard>
        <SwotCard icon={AlertTriangle} title="Faiblesses" tone="warning">
          {weaknesses.length ? weaknesses.map((item) => <QueryLine key={item.query} item={item} suffix="visibilité à améliorer" />) : <EmptyState icon={AlertTriangle} title="Aucune faiblesse nette détectée" compact />}
        </SwotCard>
        <SwotCard icon={Lightbulb} title="Opportunités" tone="info">
          {opportunities.length ? opportunities.map((item) => <QueryLine key={item.query} item={item} suffix="proche d'une position forte" />) : <EmptyState icon={Lightbulb} title="Aucune opportunité nette détectée" compact />}
        </SwotCard>
        <SwotCard icon={Users} title="Concurrence" tone="neutral">
          <div className="rounded-lg border border-border/60 p-4">
            <p className="text-sm font-medium">Comparaison des concurrents non mesurée</p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <ListCard title="Ce qui rapporte du trafic" subtitle="Requêtes organiques classées par clics">
          {trafficQueries.map((item) => <QueryLine key={item.query} item={item} suffix={`${formatNumber(item.clicks)} clics`} />)}
        </ListCard>
        <ListCard title="Pages qui captent le trafic" subtitle="Pages organiques classées par clics">
          {trafficPages.map((item) => <PageLine key={item.page} item={item} />)}
        </ListCard>
      </div>
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          ...
          <Insight title="Le levier immédiat" text={opportunities[0] ? `« ${opportunities[0].query} » cumule ${formatNumber(opportunities[0].impressions)} impressions autour d'une position ${formatPosition(opportunities[0].position)}.` : "Pas de requête à fort potentiel identifiée sur la période."} />
...
function Metric({ icon: Icon, label, value, detail }: { icon: typeof Eye; label: string; value: string; detail: string }) {
...
function QueryLine({ item, suffix }: { item: SeoQuery; suffix: string }) {
  return <div className="flex items-start justify-between gap-3 rounded-lg border border-border/60 p-3"><div className="min-w-0"><p className="truncate text-sm font-medium">{item.query}</p><p className="mt-1 text-xs text-muted-foreground">Position {formatPosition(item.position)} · CTR {formatPercent(item.ctr)} · {formatNumber(item.impressions)} impressions</p></div><Badge variant="outline" className="shrink-0 font-normal">{suffix}</Badge></div>;
}

function PageLine({ item }: { item: SeoPage }) {
...
function Insight({ title, text }: { title: string; text: string }) {
  return <div className="rounded-lg bg-muted/40 p-4"><p className="text-sm font-medium">{title}</p><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{text}</p></div>;
}
