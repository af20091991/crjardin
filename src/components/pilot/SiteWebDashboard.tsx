        ...
        <div className="grid gap-0 lg:grid-cols-[1.35fr_0.65fr]">
            ...
            <div className="mt-6 flex flex-col gap-6 sm:flex-row sm:items-center">
              ...
              <div className="grid flex-1 gap-3 sm:grid-cols-2">
                {[
                  ...
                  [FileText, "Contenus", "À construire"],
                  [MousePointerClick, "Conversion", "À construire"],
                ].map(([Icon, label, status]) => {
                  const ItemIcon = Icon as typeof Activity;
                  return (
                    <div key={String(label)} className="flex items-center gap-3 rounded-lg bg-muted/30 p-3">
          ...
          <div className="border-t bg-muted/20 p-5 lg:border-l lg:border-t-0 lg:p-6">
            ...
            <div className="mt-4 space-y-4">
              <div>
                <p className="text-sm font-medium">Aucune donnée externe branchée</p>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                  Cette première étape pose uniquement l'interface. Elle ne modifie aucune donnée
                  existante de Pilot Pro.
                </p>
              </div>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Target className="h-4 w-4" />
                Une décision claire plutôt qu'une accumulation de statistiques.
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="mt-5 flex h-44 items-center justify-center rounded-lg border border-dashed bg-muted/10 text-center">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Search Console / Analytics seront branchés dans une étape dédiée.
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="mt-4 divide-y rounded-lg border">
            {actionRows.map((label, index) => (
              <div key={label} className="flex items-center gap-3 px-3 py-3">
                ...
                <StatusPill>À alimenter</StatusPill>
              </div>
            ))}
          </div>
        </Card>
