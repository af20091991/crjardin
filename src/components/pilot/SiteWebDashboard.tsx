...
function EmptyModuleView({ title, description, icon: Icon }: { title: string; description: string; icon: typeof Search }) {
  return <Card className="p-6"><div className="flex min-h-[360px] items-center justify-center text-center"><div className="max-w-md"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-muted/60 text-primary"><Icon className="h-5 w-5" /></div><h2 className="mt-4 font-serif text-xl font-semibold">{title}</h2><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{description}</p><div className="mt-4"><StatusPill>Prochaine étape</StatusPill></div></div></div></Card>;
}
